...
export const CourseExplorer: React.FC<CourseExplorerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'content'>('content');
  ...
  // Click file (video or pdf) - Enforced without login
      ...
      try {
        ...
        if (media) {
          ...
          if (fallbackUrl) {
            ...
            alert("This content is being decrypted. Please try again in a moment.");
          }
        }
      } catch (e) {
  ...
  return (
    <div className="w-full max-w-[90rem] mx-auto px-4 sm:px-6 pt-4 pb-20 fade-in">
      {/* Top Header / Back navigation */}
      <div className="flex items-center justify-between gap-4 mb-6">
        <button
          onClick={onBack}
          className="px-3.5 py-2 bg-[#121212] hover:bg-white/10 text-stone-300 hover:text-white border border-white/10 rounded-xl text-xs font-bold transition-all btn-click-effect flex items-center gap-2"
        >
          <ArrowLeft className="w-4 h-4 text-[#FACC15]" />
          <span>Back to All Batches</span>
        </button>

        <div className="flex items-center gap-2 bg-[#121212] border border-white/10 px-3 py-1.5 rounded-lg text-xs font-semibold text-stone-300">
          <span className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse"></span>
          <span>Zero-Login Access Enforced</span>
        </div>
      </div>

      {/* Batch Banner Card */}
      <div className="bg-[#121212] border border-white/10 rounded-2xl p-5 sm:p-7 mb-8 relative overflow-hidden flex flex-col md:flex-row items-center gap-6 shadow-xl">
        <div className="w-full md:w-64 h-36 sm:h-40 shrink-0 rounded-xl overflow-hidden bg-black border border-white/10 relative group">
          <img
            src={batch.thumbnail}
            alt={batch.title}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
          <div className="absolute top-2.5 right-2.5 bg-[#10B981]/20 border border-[#10B981]/30 backdrop-blur-md px-2 py-0.5 rounded text-[9px] font-black text-[#10B981] uppercase">
            ENFORCED FREE
          </div>
        </div>
        ...
        <div className="flex-1 w-full flex flex-col justify-center">
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-[#FACC15]/10 border border-[#FACC15]/30 text-[#FACC15] text-[10px] font-bold px-2.5 py-0.5 rounded">
              {batch.category || "Next Toppers Curriculum"}
            </span>
            <span className="text-xs text-stone-400">ID: {batch.id}</span>
          </div>
          ...
          <p className="text-stone-400 text-xs sm:text-sm line-clamp-2 mb-4 leading-relaxed">
            Full curriculum unlocked with zero login barrier. Browse live sessions, PDF study notes, and assessment tests.
          </p>
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-white/5">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 btn-click-effect ${
                activeTab === 'content'
            ...
            >
              ...
              <span>Browse Folders & Content</span>
            </button>
      ...
      {/* OVERVIEW TAB */}
      {activeTab === 'overview' && (
        <div className="bg-[#121212] border border-white/10 rounded-2xl p-6 sm:p-8 mb-10 fade-in">
      ...
      {activeTab === 'content' && (
        <div>
          {/* Breadcrumbs */}
          <div className="flex items-center gap-2 overflow-x-auto hide-scroll py-2 px-3 bg-[#121212] border border-white/10 rounded-xl mb-6 text-xs">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-stone-400 font-medium text-xs">Decrypting course folder structure...</p>
            </div>
          )}

          {/* Empty State */}
          {!loading && items.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center bg-[#121212]/50 border border-white/5 rounded-2xl">
              ...
              <h3 className="text-base font-bold text-white mb-1">Folder is Empty</h3>
              <p className="text-stone-400 text-xs max-w-sm mb-4">
                No subfolders or lectures found here. Try going back to the parent folder.
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {items.map((item, idx) => {
                if (item.type === 'folder') {
                  ...
                  const badges: string[] = [];
                  if (fCount > 0) badges.push(`${fCount} FOLDERS`);
                  if (vCount > 0) badges.push(`${vCount} VIDEOS`);
                  if (pCount > 0) badges.push(`${pCount} PDFS`);
                  if (tCount > 0) badges.push(`${tCount} TESTS`);
                  ...
                  return (
                    ...
                    >
                      <div className="h-[135px] w-full relative overflow-hidden bg-[#161618] border-b border-white/5 flex items-center justify-center">
                        {item.data?.thumbnail ? (
                          <img
                      ...
                      <div className="p-3.5 flex-1 flex flex-col justify-between bg-[#121212] group-hover:bg-[#18181a] transition-colors">
                        <h3 className="text-white font-semibold text-xs leading-snug line-clamp-2">
                          {item.title}
                        </h3>
                        <div className="text-[10px] text-[#FACC15] font-bold tracking-wider uppercase">
                          {badges.length > 0 ? badges.join(' • ') : 'EMPTY FOLDER'}
                        </div>
                      </div>
                    </div>
                  );
                ...
                if (item.type === 'file') {
                      ...
                      >
                        ...
                        <div className="p-3.5 flex-1 flex flex-col justify-between bg-[#121212] group-hover:bg-[#18181a] transition-colors">
                          ...
                          <div className="text-[10px] text-stone-500 font-medium flex items-center gap-1">
                            ...
                            <span>{durationText || 'Recorded Lecture'}</span>
                            {isResolving && <span className="text-[#FACC15] font-bold ml-auto animate-pulse">Enforcing...</span>}
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-[135px] w-full bg-[#18181b] flex items-center justify-center border-b border-white/5">
                        {isResolving ? (
                      ...
                      <div className="p-3.5 flex-1 flex flex-col justify-between bg-[#121212] group-hover:bg-[#18181a] transition-colors">
                        ...
                        <div className="text-[10px] text-red-400 font-bold tracking-wider flex items-center gap-1">
                          ...
                          <span>{isResolving ? 'DECRYPTING PDF...' : 'TAP TO VIEW PDF'}</span>
                        </div>
                      </div>
                    </div>
                  );
                      ...
                      <div className="flex items-start sm:items-center gap-3.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[9px] font-black px-2 py-0.5 rounded tracking-widest uppercase">
                              CBT ASSESSMENT (NO LOGIN)
                            </span>
                          </div>
      ...
      {pdfModalData && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-5">
          <div className="bg-[#121212] border border-white/10 w-full max-w-5xl h-[90vh] rounded-2xl flex flex-col overflow-hidden shadow-2xl">
            {/* Modal Header */}
